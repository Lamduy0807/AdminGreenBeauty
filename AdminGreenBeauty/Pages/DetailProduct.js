...
const DetailProduct = ({navigation, route}) => {
  ...
  const GetData = () => {
    setProduct({
      ...
      brand: route.params.product.brand,
    });
  ...
  async function getCategoryId(id) {
    const apiGetDetailById = 'http://10.0.2.2:8000/category/' + id;
    try {
  ...
  async function getListImgbyId(id) {
    const apiGetDetailById = 'http://10.0.2.2:8000/img/?product=' + id;
    try {
      let response = await fetch(apiGetDetailById, {
        method: 'GET',
      });
  ...
  const AddHadle = () => {
    ...
    postProduct(route.params.product.id)
      .then(re => {
        console.log('list ', listImg);
        ...
        Alert.alert('Thông báo', 'put success');
      })
      .catch(() => {
        alert('Error', 'Put unsuccessfull');
      });
  };
    ...
    <SafeAreaView style={{flex: 1}}>
      <ScrollView style={{flex: 1}}>
        ...
        <View
          ...
          <Text
            style={{marginLeft: 10, marginRight: scale(5), fontWeight: 'bold'}}>
            Active
          </Text>
          <CheckBox
            disabled={false}
            ...
            renderItem={({item, index}) => {
              ...
              return (
                <View style={{alignItems: 'flex-end'}}>
                  <Image
                    style={styles.Img1}
                    source={{uri: item.img.replace('127.0.0.1', '10.0.2.2')}}
                  />
                  <TouchableOpacity
                    style={{
